...
void mvAnalysis::analyze(c2ga::mv X, int intFlags/* = 0 */,  double epsilon/* = DEFAULT_EPSILON */, 
	...
	m_mvType = mvType(X, (c2ga::mv::Float)epsilon);
	...
	// pseudo scalar
	else if (m_mvType.m_gradeUsage == GRADE_4) {
	...
	c2ga::mv::Float _opNiX = _Float(norm_e(op(ni, X)));
	c2ga::mv::Float _ipNiX = _Float(norm_e(lcont(ni, X)));
	c2ga::mv::Float _X2 = _Float(norm_r2(X));
...
void mvAnalysis::analyzeRound(c2ga::mv X, int intFlags /* = 0 */, double epsilon /* = DEFAULT_EPSILON */) {
	...
	m_pt[0] = vectorToE3GA(_vectorE2GA(location));
	m_sc[0] = sqrt(fabs(radius2)) * ((radius2 < 0) ? -1.0f : 1.0f);
	m_sc[1] = weight;

	// factor attitude:
	if ((Agrade == 3) || (Agrade == 1)) { 
		// (dual) circle (WHAT ABOUT DUAL POINTS WITH 0 RADIUS)? 
		m_vc[0] = e3ga::_vector(e3ga::e1);
		m_vc[1] = e3ga::_vector(e3ga::e2);
		m_vc[1] = e3ga::_vector(e3ga::e3);
	}
	else if (Agrade == 2) {
		// point pair
		m_vc[0] = vectorToE3GA(_vectorE2GA(unit_e(lcont(no, attitude))));
	}
...
void mvAnalysis::analyzeTangent(const mv &X, int intFlags /* = 0 */, double epsilon /* = DEFAULT_EPSILON */) {
	...
	// ************* format of tangent ***************
	// m_pt[0] = location
	// m_vc[0] .. m_vc[2] = unit 2D vector basis for attitude
	// m_sc[0] = weight
...
void mvAnalysis::analyzeFlat(const mv &X, const normalizedPoint &probe, int intFlags /* = 0 */, double epsilon /* = DEFAULT_EPSILON */) {
	...
	m_type[2] = FLAT;
	int Agrade = m_mvType.m_grade;
	if (m_flags & FLAG_DUAL) Agrade = 4 - Agrade;
